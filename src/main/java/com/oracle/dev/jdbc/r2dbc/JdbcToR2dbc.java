...
/**
 * <p>
 * This class presents a series of code examples demonstrating usages of both
 * JDBC and R2DBC APIs that produce identical database interactions. The target
 * audience for these examples are programmers seeking to learn learn how R2DBC
 * can accomplish the same work as JDBC.
 * </p>
 * <p>
 * Examples in this class read the database connection configuration from a file
 * named "config.properties" that exists in the current directory when this demo
 * is run. There is an example config.properties file in the /sample directory
 * (relative to the root this repository).
 * </p>
 * <b> List of Examples </b>
 * <ol>
 * <li>Configuring Database Connections
 * <ul>
 ...
 * </ul>
 * </li>
 * <li>Attempt to INSERT a row which may already exist. If the row already
 ...
 * </ul>
 * </li>
 * <li>Attempt to UPDATE a row which may not exist. If the row doesn't exist,
 * then INSERT it.
 * <ul>
 ...
 * already exists, then repeatedly attempt the UPDATE and INSERT until one
 * succeeds.
 * <ul>
...
public final class JdbcToR2dbc {
	...
	static DataSource configureJdbc() throws SQLException {
		...
		dataSource.setServiceName(DatabaseConfig.getDatabase());
	...
	static ConnectionFactory configureR2dbc() {
		...
		return ConnectionFactories
				.get(ConnectionFactoryOptions.builder().option(DRIVER, "oracle").option(HOST, DatabaseConfig.getHost())
						.option(PORT, DatabaseConfig.getPort()).option(DATABASE, DatabaseConfig.getDatabase())
						.option(USER, DatabaseConfig.getUser()).option(PASSWORD, DatabaseConfig.getPassword()).build());
	...
	static String queryJdbc(java.sql.Connection connection) throws SQLException {
		...
		try (java.sql.Statement statement = connection.createStatement()) {
			ResultSet resultSet = statement.executeQuery("SELECT 'Hello, JDBC!' FROM sys.dual");
	...
	static Publisher<String> queryR2dbc(io.r2dbc.spi.Connection connection) {
		...
		return Flux.from(connection.createStatement("SELECT 'Hello, R2DBC!' FROM sys.dual").execute())
	...
	static int insertJdbc(java.sql.Connection connection) throws SQLException {
		...
		try (PreparedStatement preparedStatement = connection
				.prepareStatement("INSERT INTO JdbcToR2dbcTable(id, value) VALUES (?, ?)")) {
	...
	static Publisher<Long> insertR2dbc(io.r2dbc.spi.Connection connection) {
		...
		return Flux.from(connection.createStatement("INSERT INTO JdbcToR2dbcTable(id, value) VALUES (?, ?)").bind(0, 0)
				.bind(1, "R2DBC").execute()).flatMap(Result::getRowsUpdated);
	...
	static int updateJdbc(java.sql.Connection connection) throws SQLException {
		...
		try (PreparedStatement preparedStatement = connection
				.prepareStatement("UPDATE JdbcToR2dbcTable SET value = ? WHERE id = ?")) {
	...
	static Publisher<Long> updateR2dbc(io.r2dbc.spi.Connection connection) {
		...
		return Flux.from(connection.createStatement("UPDATE JdbcToR2dbcTable SET value = ? WHERE id = ?")
				.bind(0, "R2DBC").bind(1, 0).execute()).flatMap(Result::getRowsUpdated);

	}

	/**
	 * Tries to update a row with JDBC. If the updated row does not exist, then this
	 * method tries to insert it. This method performs the same database
	 * interactions as {@link #tryUpdateR2dbc(io.r2dbc.spi.Connection)}
	 * 
	...
	/**
	 * Tries to update a row with R2DBC. If the updated row does not exist, then
	 * this method tries to insert it. This method performs the same database
	 * interactions as {@link #tryUpdateJdbc(java.sql.Connection)}
	 * 
	 * @param connection Database connection
	 * @return {@code Publisher} emitting the count of updated rows.
	 */
	static Publisher<Long> tryUpdateR2dbc(io.r2dbc.spi.Connection connection) {
		...
		// Try to update the row
		return Flux.from(updateR2dbc(connection)).flatMap(updateCount -> {
	...
	/**
	 * Tries to insert a row with JDBC. If the INSERT fails because the row already
	 * exists, this method tries to update it. This method performs the same
	 * database interactions as {@link #tryInsertR2dbc(io.r2dbc.spi.Connection)}.
	 * 
	 * @param connection Database connection
	 * @return Count of updated rows
	 */
	...
	/**
	 * Tries to insert a row with R2DBC. If the INSERT fails because the row already
	 * exists, this method tries to update it. This method performs the same
	 * database interactions as {@link #tryInsertJdbc(java.sql.Connection)}.
	 * 
	 * @param connection Database connection
	 * @return {@code Publisher} emitting the count of updated rows.
	 */
	static Publisher<Long> tryInsertR2dbc(io.r2dbc.spi.Connection connection) {
		...
		// Try to insert the row
		return Flux.from(insertR2dbc(connection)).onErrorResume(R2dbcException.class, r2dbcException -> {
	...
	/**
	 * Tries to insert a row with JDBC. If the Uses JDBC to execute a loop where a
	 * failed INSERT is handled by executing an UPDATE instead. The loop exits when
	 * the INSERT succeeds or the UPDATE successfully updates a row. This method
	 * performs the same database interactions as the
	 * {@link #loopR2dbc(io.r2dbc.spi.Connection)} method.
	 * 
	 * @param connection Database connection
	 * @return Count of updated rows
	 * @throws SQLException If the database interaction fails with an unexpected
	 *                      error.
	 */
	...
	/**
	 * <p>
	 * Uses R2DBC to execute a loop where a failed INSERT is handled by executing an
	 * UPDATE instead. The loop exits when the INSERT succeeds or the UPDATE
	 * successfully updates a row. This method performs the same database
	 * interactions as the {@link #loopJdbc(java.sql.Connection)} method.
	 * </p>
	 * <p>
	 * Synchronous programming would typically implement a loop using {@code while}
	 * or {@code for} statements. This synchronous model works well for
	 * {@linkplain #loopJdbc(java.sql.Connection) JDBC calls} where an iteration of
	 * the loop blocks until the call completes, and the result of call can then be
	 * evaluated to decide if the loop should repeat for another iteration.
	 * </p>
	 * <p>
	 * With asynchronous programming, {@code while} and {@code for} can not be used
	 * to execute database calls in a loop. The exit condition of the loop can not
	 * be evaluated until the result of the database call is known. So, instead of
	 * using {@code while} or {@code for}, this method implements the loop using
	 * recursion. If the INSERT results in failure, or if the UPDATE results in a
	 * row count of zero, then the {@code Publisher} returned by this method
	 * completes with the result of another {@code Publisher} returned by a
	...
	static Publisher<Long> loopR2dbc(io.r2dbc.spi.Connection connection) {
		...
		// Try to update the row, or insert it if it does not exist
		return Flux.from(tryUpdateR2dbc(connection)).onErrorResume(R2dbcException.class, r2dbcException -> {
	...
	/**
	 * Executes all examples with JDBC. The {@link #createTable()} method should be
	...
	static void useJdbc() {
		try (java.sql.Connection connection = configureJdbc().getConnection()) {
			...
			System.out.println(tryInsertJdbc(connection) + " row(s) updated.");
			System.out.println(tryUpdateJdbc(connection) + " row(s) updated.");
			System.out.println(loopJdbc(connection) + " row(s) updated.");
		} catch (SQLException sqlException) {
	...
	/**
	 * Executes all examples with R2DBC. The {@link #createTable()} method should be
	 * called prior to invoking this method.
	 */
	static void useR2dbc() {
		Flux.usingWhen(configureR2dbc().create(),
				connection -> Flux.from(queryR2dbc(connection)).doOnNext(System.out::println)
						.thenMany(tryInsertR2dbc(connection))
						.doOnNext(updateCount -> System.out.println(updateCount + " row(s) updated."))
						.thenMany(tryUpdateR2dbc(connection))
						.doOnNext(updateCount -> System.out.println(updateCount + " row(s) updated."))
						.thenMany(loopR2dbc(connection))
						.doOnNext(updateCount -> System.out.println(updateCount + " row(s) updated.")),
				io.r2dbc.spi.Connection::close).blockLast(Duration.ofSeconds(30));
	}
	...
	static void createTable() {
		Flux.usingWhen(configureR2dbc().create(),
				connection -> Flux.from(connection
						.createStatement(
								"CREATE TABLE JdbcToR2dbcTable (" + "id NUMBER PRIMARY KEY, value VARCHAR2(128))")
						.execute()).flatMap(Result::getRowsUpdated).onErrorReturn(error ->
				// Check if the error is ORA-00955 for creating a table that already
				// exists. If so, then ignore it.
				error instanceof R2dbcException && ((R2dbcException) error).getErrorCode() == 955, 0L),
				io.r2dbc.spi.Connection::close).blockLast(Duration.ofSeconds(15));
	}
	...
	static void dropTable() {
		Flux.usingWhen(configureR2dbc().create(),
				connection -> Flux.from(connection.createStatement("DROP TABLE JdbcToR2dbcTable").execute())
				...
				.doOnError(System.out::println).onErrorReturn(1L).blockLast(Duration.ofSeconds(15));
	}
